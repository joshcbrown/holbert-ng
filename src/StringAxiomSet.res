...
let derive = (group: judgeGroup, allGroups: array<judgeGroup>): Rule.t => {
  let allVars =
    allGroups
...
let deserialise = (str: string, ~imports as _: Ports.t) => {
  ...
  getBase(str)->Result.map(raw => {
    ...
    allGroups->Array.forEach(group => {
      ...
      derived->Dict.set(`${group.name}_induct`, derive(group, []))
      derived->Dict.set(`${group.name}_mutualInduct`, derive(group, allGroups))
    })
